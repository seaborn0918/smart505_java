public class ArrayCreateEx1 {
  public static void main(String[] args) {
    ...
    // 다른점
    // 1. 자바의 배열은 생성 시 지정된 크기를 변경할 수 없음
    // 2. 자바의 배열은 동일한 데이터 타입만 저장할 수 있음
    ...
    // 배열 타입의 변수만 먼저 생성하고 필요에 따라서 배열 객체를 생성하여 사용하는 방식
    // 배열의 크기가 결정되지 않았기 때문에 나중에 배열 객체 생성 시 크기를 결정할 수 있도록 반드시 new 키워드를 사용해야 함
    ...
    System.out.println("------- 배열의 기본값 --------");

    byte bVal;
    int iVal;

    // 초기화되지 않은 변수르,ㄹ 사용하기 때문에 오류가 발생함
    ...
    System.out.println("byte 타입의 배열 bArr의 0번 인텍스 : " + bArr[0]);
    System.out.println("int 타입의 배열 aArr의 0번 인텍스 : " + aArr[0]);
    System.out.println("double 타입의 배열 dArr의 0번 인텍스 : " + dArr[0]);
    System.out.println("boolean 타입의 배열 boArr의 0번 인텍스 : " + boArr[0]);
    System.out.println("String 타입의 배열 sArr의 0번 인텍스 : " + sArr[0]);
    ...
    // numArr1.length = 10 // length는 읽기 전용
  }
}
